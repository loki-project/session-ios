...
public final class Storage : NSObject {
    ...
    private static var owsStorage: OWSPrimaryStorage {
        return OWSPrimaryStorage.shared()
    }
    
    private static var writeConnection: YapDatabaseConnection {
        return owsStorage.dbReadWriteConnection
    }
    
    /// Used internally by `dbReadWriteConnection` for `readWrite` operations.
    private static var connectionQueue: DispatchQueue {
        return writeConnection.lk_getConnectionQueue()
    }
    
    /// Used internally by `dbReadWriteConnection` for `readWrite` operations.
    private static var writeQueue: DispatchQueue {
        return owsStorage.lk_getWriteQueue()
    }
    
    private static var isWriteQueue: Bool {
        return getQueueLabel() == getQueueLabel(writeQueue)
    }
    ...
    public static func write(with block: @escaping (YapDatabaseReadWriteTransaction) -> Void) {
        if let writeTransaction = writeTransaction {
