...
public final class MultiDeviceProtocol : NSObject {
    ...
    private static func copy(_ messageSend: OWSMessageSend, for destination: MultiDeviceDestination, with longTermSeal: Resolver<Void>) -> Promise<OWSMessageSend> {
        ...
        DispatchQueue.main.async {
            ...
            let messageSend = OWSMessageSend(message: messageSend.message, thread: messageSend.thread, recipient: recipient,
                senderCertificate: senderCertificate, udAccess: recipientUDAccess, localNumber: messageSend.localNumber, success: {
                longTermSeal.fulfill(())
            }, failure: { error in
                longTermSeal.reject(error)
            })
    ...
    private static func sendMessage(_ messageSend: OWSMessageSend, to destination: MultiDeviceDestination, in transaction: YapDatabaseReadTransaction) -> Promise<Void> {
        ...
        return threadPromise.then(on: OWSDispatch.sendingQueue()) { thread -> Promise<Void> in
            ...
            if !shouldSendAutoGeneratedFR {
                copy(messageSend, for: destination, with: seal).done(on: OWSDispatch.sendingQueue()) { messageSendCopy in
